...
const Home: React.FC = () => {
    ...
    const handleTripSubmit = async (input: TripInput) => {
        ...
        setError(null);
        setItinerary(null);
        setLastInput(input); // Store the input for regeneration
    ...
    const handleRegenerate = async () => {
        ...
        try {
            const result = await generateItinerary(lastInput);
            setItinerary(result);
        } catch (err) {
            console.error(err);
            setError("We couldn't regenerate your trip plan. Please try again.");
        } finally {
